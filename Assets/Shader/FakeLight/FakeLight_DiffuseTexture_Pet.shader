...
Shader "FakeLight/Diffuse/Texture/Pet"
{
	Properties
	{
	    ...
	    _HighShadow("HighShadow", Range(-1, 1)) = 0.1
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f_base i) : COLOR
			{
				...
				fixed4 clr = mainColor * _FakeLightColor * min(diffuse + _Brightness, 1);//0.2
		...
		Pass
		{
			...
			float4 vert(float4 vertex: POSITION) : SV_POSITION
			{
				...
				vt.y = _HighShadow;//0.1;//使陰影保持在接收平面上  
