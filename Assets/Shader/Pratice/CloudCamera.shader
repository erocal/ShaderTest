    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float random(float3 value, float3 dotDir)
            {
                ...
                random = frac(sin(random) * 1234594.43212);// frac保證我們最後取得的值在0 - 1
                return random;
            }

            float3 random3d(float3 value) 
            {
                return float3(  random(value, float3(12.898, 45.12, 89.7298)),
                                random(value, float3(14.898, 45.12, 64.7298)),
                                random(value, float3(43.898, 45.45, 57.453)));
            }
            ...
            {
                fixed3 lighting = fixed3(1, 1, 1) * 1.3 + 0.5 * fixed3(0.7, 0.5, 0.3) * diffuse;
            ...
            #define MARCH(steps, noiseMap, cameraPos, viewDir, bgcol, sum, depth, t) { \
                ...
                { \
                    if (t > depth) break; \// 如果t大於深度，那麼您就超出了相機目前的範圍看到或你已經經過的深度
                    float3 pos = cameraPos + t * viewDir; \// 如果t小於深度，我們可以計算下一步或我們要走多遠步數
                                                            \//即更新pos，即與相機的距離
                                                            \//所以 pos 是相機的位置加上 t 乘以視圖方向
                    ...
                    \
                    float density = noiseMap(pos); \// 計算密度
                    if (density > 0.01) \
                    { \
            ...
            #define NOISEPROC(N, P) 1.75 * N * saturate((_MaxHeight - P.y)/_FadeDist)// 確保在雲端中獲得的值的範圍
            ...
            fixed4 raymarch(float3 cameraPos, float3 viewDir, fixed4 bgcol, float depth)
            {
                ...
                return clamp(col, 0.0, 1.0);// clamp 限制在0和1之間
            }
