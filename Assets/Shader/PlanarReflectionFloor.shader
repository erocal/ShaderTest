Shader "Custom/PlanarReflectionFloor"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            // 添加一个 smoothstep 函数，控制 reflectionColor 的叠加
            float blendFactor = smoothstep(0.4, 1.0, reflectionColor.r); // 调整参数以控制叠加的范围

            // 將_MainTex的顏色和反射的顏色進行疊加，但reflectionColor 只在白色部分叠加
            fixed4 finalColor = mainTexColor + reflectionColor * blendFactor;

            // 其他部分保持不變
            o.Albedo = finalColor.rgb;
            o.Normal = UnpackNormal(tex2D(_myBump, IN.uv_myBump) * _myTextureBumpSlider);
