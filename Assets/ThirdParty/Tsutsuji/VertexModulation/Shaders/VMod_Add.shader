...
Shader "Tsutsuji/VertexModulation_Add" {
    Properties {
        ...
        [HDR]_OutsideColorFlash ("Outside Color Flash", Color) = (0,0,0,1)
        [HDR]_InsideColorFlash ("Inside Color Flash", Color) = (1,1,1,1)
        _NoiseTexture ("Noise Texture", 2D) = "white" {}
        _NoiseScrollSpeed ("Noise Scroll Speed", Range(-1, 1)) = 0
        _NoiseRotateSpeed ("Noise Rotate Speed", Range(-1, 1)) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_4836 = _Time;
                float node_3144 = (node_3443.g*5.0*_NoiseRotateSpeed);
                float node_5344_ang = node_4836.g;
                ...
                float2 node_5344_piv = (float2(sin(node_3144),cos(node_3144))*0.5+0.5);
...
////// Emissive:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_3443 = _Time;
                float4 node_9206 = _Time;
                float node_3144 = (node_3443.g*5.0*_NoiseRotateSpeed);
                float node_5344_ang = node_9206.g;
                float node_5344_spd = (_NoiseRotateSpeed*2.0);
                float node_5344_cos = cos(node_5344_spd*node_5344_ang);
                float node_5344_sin = sin(node_5344_spd*node_5344_ang);
                float2 node_5344_piv = (float2(sin(node_3144),cos(node_3144))*0.5+0.5);
